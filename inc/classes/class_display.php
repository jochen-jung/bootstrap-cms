...
class display {
  ...
  */
  ...
  function StartTab($name, $icon = '') {
    ...
    $this->TabNames[] = $name;
    $MainContent .= '<div class="tab-pane" id="tabNr'. (int)$this->CurrentTab .'">';
    $this->CurrentTab++;
  }
  ...
  function EndTabs() {
    ...
    foreach ($this->TabNames as $key => $name) {
      $items .= '<li><a href="#tabNr'. $key .'" data-toggle="tab">'. $name .'</a></li>';
    }
    $out .= '<ul class="nav nav-tabs" id="tabGroup">'. $items .'</ul>';
    $out .= '<div class="tab-content">';

    $sel = ($_GET['tab'])? '[href=#tabNr'. (int)$_GET['tab'] .']' : ':first';

    $framework->add_js_code('$(function () {
      $(\'#tabGroup a'. $sel .'\').tab(\'show\');
    })');
...
#		$MainContent .= "    <li class='ui-state-default ui-corner-top".$additional."'><a href='".$link."' title='".$title."'><em>".$content."</em></a></li>\n";
  ...
  function AddDoubleRow($key, $value, $id = NULL) {
    ...
    $this->AddContentLine($smarty->fetch('tpl/row_double.tpl'));
  }
  ...
  function AddCheckBoxRow($name, $key, $text, $errortext, $optional = NULL, $checked = NULL, $disabled = NULL, $val = NULL, $additionalHTML = NULL) {
    ...
    $smarty->assign('label', $key);
    $value = '<input id="'. $name .'" name="'. $name .'" type="checkbox" class="checkbox" value="'. $val .'" '. $checked .' '. $disabled .' '. $additionalHTML .' />';
    $value .= '<label class="inline-label" for="'. $name .'">'. $text .'</label>'. $errortext;
    $smarty->assign('input', $value);
    $this->AddContentLine($smarty->fetch('tpl/form_input_row.tpl'));
  }
  ...
  function AddTextFieldRow($name, $key, $value, $errortext, $size = NULL, $optional = NULL, $not_changeable = NULL, $maxlength = NULL) {
    ...
    $smarty->assign('id', $name);
    $smarty->assign('label', $key);
    $smarty->assign('input', '<input type="text" id="'. $name .'" name="'. $name .'"'
        .' class="input-xlarge form'. $optional .'" size="'. $size .'"'
        . $not_changeable .' value="'. $value .'"'. $maxlength .' />'. $errortext);
    $this->AddContentLine($smarty->fetch('tpl/form_input_row.tpl'));
  }
  ...
  function AddPasswordRow($name, $key, $value, $errortext, $size = NULL, $optional = NULL, $additional = NULL) {
    ...
    $smarty->assign('id', $name);
    $smarty->assign('label', $key);
    $smarty->assign('input', '<input type="password" id="'. $name .'" name="'
      . $name .'" class="form'. $optional .'" size="'. $size .'" value="'
      . $value .'" '. $additional .' />'. $errortext);
    $this->AddContentLine($smarty->fetch('tpl/form_input_row.tpl'));
  }
  ...
  function AddTextAreaMailRow($name, $key, $value, $errortext, $cols = NULL, $rows = NULL, $optional = NULL, $maxchar = NULL) {
    ...
    $smarty->assign('id', $name);
    $smarty->assign('label', $key);
    $smarty->assign('input', '<textarea name="'. $name .'" id="'
       . $name .'" class="form'. $name .'" cols="'. $cols .'" rows="'
       . $rows .'" onKeyUp="TextAreaPlusCharsLeft(this, document.'
       . $this->form_name .'.'. $name .'_chr, '. $maxchar
       .'); AddaptTextAreaHeight(this)">'. $value .'</textarea>'. $errortext
       . '<br><a href="index.php?mod=popups&action=ls_row_textareamail_popup&design=popup&form='. $this->form_name .'&textarea='. $name .'" onclick="OpenWindow(this.href, \'TextFormatSelect\'); return false">Variablen einfügen</a>'
       );
    $this->AddContentLine($smarty->fetch('tpl/form_input_row.tpl'));
  }
  ...
  function AddTextAreaRow($name, $key, $value, $errortext, $cols = NULL, $rows = NULL, $optional = NULL) {
    ...
    $smarty->assign('id', $name);
    $smarty->assign('label', $key);
    $smarty->assign('input', '<textarea name="'. $name .'" id="'. $name
      .'" class="form'. $name .'" cols="'. $cols .'" rows="'
      . $rows .'" onKeyUp="AddaptTextAreaHeight(this)">'
      . $value .'</textarea>'. $errortext);
    $this->AddContentLine($smarty->fetch('tpl/form_input_row.tpl'));
  }
  ...
  function AddTextAreaPlusRow($name, $key, $value, $errortext, $cols = NULL, $rows = NULL, $optional = NULL, $maxchar = NULL) {
    ...
    $buttons .= " ". $this->FetchIcon('index.php?mod=popups&action=textareaplus_preview&design=popup&textareaname='. $name .'" onclick="javascript:OpenPreviewWindow(this.href, document.'. $this->form_name .'); return false;', 'eye-open', t('Vorschau'));
    $buttons .= '</div>';
    ...
    $buttons .= " ". $this->FetchIcon("javascript:OpenWindow('index.php?mod=popups&amp;action=ls_row_textareaplus_popup&amp;design=popup&amp;form={$this->form_name}&amp;textarea={$name}', 'TextFormatSelect')", 'smiley', t('Smiley einfügen'));
    $buttons .= " ". $this->FetchIcon("javascript:OpenWindow('index.php?mod=popups&amp;action=ls_row_textareaplus_link&amp;design=popup&amp;form={$this->form_name}&amp;textarea={$name}', 'TextFormatSelect')", 'hand-right', t('Internen Link einfügen'));
    $buttons .= " ". $this->FetchIcon("javascript:InsertCode(document.{$this->form_name}.{$name}, '[c]', '[/c]')", 'indent-left', t('Code'));
    $buttons .= " ". $this->FetchIcon("javascript:InsertCode(document.{$this->form_name}.{$name}, '[img]', '[/img]')", 'picture', t('Bild'));
    $buttons .= '</div>';
    ...
    $this->AddContentLine($smarty->fetch('tpl/form_textarea_plus.tpl'));
  }
  ...
  function AddFieldsetStart($name) {
    ...
    $MainContent .=  '<fieldset><legend>'. $name .'</legend>';
  }
  ...
  function AddFormSubmitRow($text, $close = true, $name = "imageField") {
    $this->AddDoubleRow('&nbsp;', '<button type="submit" class="btn" name="'. $name .'">'. $text .'</button>');
...
*/
  ...
  function FetchSpanButton($text, $link, $title = null, $target = null) {
    $attr = '';
    if ($title) $attr .= t($title);
    if ($target) $attr .= ' target="_blank"';
    return '<a class="btn" href="'. $link .'"'. $attr .'>'. $text .'</a>';
  }

  function FetchCssButton($text, $link, $title = null, $target = null) {
    return $this->FetchSpanButton($text, $link, $title, $target);
  }
  
  function FetchIcon($link, $picname, $hint = null, $target = null, $align = 'left') {
    global $smarty;
    
    if (in_array($picname, array(
      'bold', 'italic', 'picture', 'eye-open', 'indent-left', 'smiley', 'hand-right', 'edit'
    ))) {
      $ret = '<span class="glyphicon glyphicon-'. $picname .'"></span>';
      if ($link) $ret = '<a class="btn" href="'. $link .'" title="'. $hint .'">'. $ret .'</a>';
      return $ret;
    }
