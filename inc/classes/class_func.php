 ...
 * @author ls_admin
 * @version $Id: class_func.php 2359 2012-04-28 07:42:54Z Jochen.Jung $
 * @access public
...
class func {
  ...
  function GeneralDialog($type, $text, $link_target = '', $JustReturn = 0, $link_type = '') {
    ...
    if ($link_target != NO_LINK) {
      ...
      $text .= '<p>'
        . $dsp->FetchCssButton($link_text, $link_target, $link_description)
        .'</p>';
    }
    ...
    switch ($type) {
      case 'confirmation': $type = 'alert-success'; break;
      case 'information': $type = 'alert-info'; break;
      case 'error': $type = 'alert-error'; break;
      default: $type = ''; break;
    }
    $smarty->assign('alertType', $type);
    
    $alert = $smarty->fetch('tpl/alert.tpl');
    if ($JustReturn) $FrameworkMessages .= $alert;
    else $dsp->AddContentLine($alert);
  }
  ...
  function page_split($current_page, $max_entries_per_page, $overall_entries, $working_link, $var_page_name) {
      ...
      switch ($_FILES[$source_var]['error']) {
        ...
        default:
          ...
          if (move_uploaded_file($_FILES[$source_var]['tmp_name'], $targetUniq)) {
              chmod ($targetUniq, octdec($config['system']['chmod_file']));
              return $targetUniq;
          } else {
    ...
    function CreateDir($dir) {
        ...
        if (!is_dir($dir)) {
            mkdir($dir, octdec($config['system']['chmod_dir']));
            #chmod($dir, octdec($config['system']['chmod_dir']));
        }
    }
