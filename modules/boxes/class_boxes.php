 ...
 * @copyright 2008
 * @version $Id: class_boxes.php 2240 2010-03-12 15:05:57Z webmaster@kiddiboy.de $
 * @access public
 */
class boxes {
   ...
   */
    function add_menuitem($caption, $link = '', $hint = '', $level = 0, $requirement = 0, $highlighted = 0, $id = '') {
      ...
      if ($highlighted) $class = "active";
      else $class = '';
      if ($link != "") {
          $tmp_link = '<a href="'.$func->AllowHTML($link).'" class="'.$link_class.'" title="'. $func->AllowHTML($hint) .'">'.$caption.$box_row_hint.'</a>';
      }
   ...
   */
    function LinkItem($link, $caption, $class = "", $hint='') {
        global $func;
        if ($link != "") {
            $out = '<a href="'.$link.'" class="'.$class.'" title="'. $func->AllowHTML($hint) .'">'.$caption.$box_row_hint.'</a>';
            return $out;
        } else return $caption;
    }
