...
*/
    ...
    if ($this->config['EntriesPerPage'] and ($count_rows['count'] > $this->config['EntriesPerPage'])) {
      ...
      if ((int)$_GET['ms_page'] > 0) {
        $pages .= '<li>'. $link_start . $link . ($_GET['ms_page'] - 1) . $link_end .'«</a></li>';
      } else $pages .= '<li class="disabled"><a href="#">«</a></li>';
      
      // First page link
      if ($_GET['ms_page'] > 4) {
        $pages .= '<li>'. $link_start . $link . '0' . $link_end .'1</a></li> ... ';
        $i = $_GET['ms_page'] - 3;
      } else $i = 0;

      // Direct page link
      while($i < $count_pages and $i < ($_GET['ms_page'] + 4)) {
        if ($_GET['ms_page'] == $i) $pages .= '<li class="active">';
        else $pages .= '<li>';
        $pages .= $link_start . $link . $i . $link_end . ($i + 1) .'</a></li>';
        $i++;
      }
      ...
      if ($i < $count_pages) {
        ...
        $pages .= '<li>'. $link_start . $link . ($count_pages - 1) . $link_end . $count_pages .'</a></li>';
      }
      ...
      while($line = $db->fetch_array($res)) { // Start: Row
        ...
        foreach ($this->icon_field as $current_field) {
          ...
          if (!$current_field['callback'] or call_user_func($current_field['callback'], $line[$select_id_field])) {
            ...
            // Mapping to new bootstrap icon names
            /*
            //if ($current_field['icon_name'] == 'edit') $current_field['icon_name'] = 'edit';
            if ($current_field['icon_name'] == 'delete') $current_field['icon_name'] = 'remove';
            if ($current_field['icon_name'] == 'unlocked') $current_field['icon_name'] = 'lock';
            if ($current_field['icon_name'] == 'locked') $current_field['icon_name'] = 'lock';
            if ($current_field['icon_name'] == 'send_mail') $current_field['icon_name'] = 'envelope';
            if ($current_field['icon_name'] == 'change_pw') $current_field['icon_name'] = 'tower';
            if ($current_field['icon_name'] == 'assign') $current_field['icon_name'] = 'share-alt';
            if ($current_field['icon_name'] == 'details') $current_field['icon_name'] = 'search';
            */
