...
if (!$user_data['userid']) $func->error(t('Dieser Benutzer existiert nicht'));
  ...
  // Address
  $address = '';
  if (($user_data['street'] != '' or $user_data['hnr']) and ($auth['type'] >= 2 or ($auth['userid'] == $_GET['userid'])))
      $address .= $user_data['street'] .' '. $user_data['hnr'] .', ';
  if (($user_data['plz'] != '' or $user_data['city']) and ($auth['type'] >= 2 or $auth['userid'] == $_GET['userid']))
  ...
  $mail = '<table width="100%" cellspacing="0" cellpadding="0"><tr><td>';
      if ($auth['type'] >= 2 or $auth['userid'] == $_GET['userid']) {
